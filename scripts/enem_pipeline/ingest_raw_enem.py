...
# MAPEAMENTO EXATO BASEADO NO ESQUEMA DA BASEDOSDADOS(?)
ALIASES = {
    ...
    # Colunas que PERDERAM o prefixo tp_
    ...
    # Colunas que MANTÊM o prefixo tp_
...
def _fetch_available_columns(billing_project_id: str, credentials) -> set[str]:
    ...
    try:
        ...
        found = set(cols["column_name"].astype(str).tolist())
        print(f"[INFO] Esquema detectado com {len(found)} colunas.")
        return found
    except Exception as e:
        print(f"[ERROR] Falha ao ler esquema: {e}. Usando modo defensivo.")
        return set()
...
def _build_select_list(available_cols: set[str]) -> str:
    ...
    for canonical in REQUIRED_CANONICAL_COLUMNS:
        ...
        #  Tenta aliases manuais mapeados 
        elif canonical in ALIASES:
            matched = next((a for a in ALIASES[canonical] if a in available_cols), None)
            if matched:
                select_exprs.append(f"{matched} AS {canonical}")
            else:
                
                fallback = ALIASES[canonical][0]
                select_exprs.append(f"{fallback} AS {canonical}")
...
def run(config_path: Path) -> None:
    ...
    for year in years:
        print(f"[INFO] Baixando ENEM {year} do BigQuery...")
        ...
        del df # Limpeza de memória obrigatória para o GitHub Actions
