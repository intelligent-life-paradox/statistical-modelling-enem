...
def _fit_causal_model(y: np.ndarray, t: np.ndarray, x: np.ndarray, random_seed: int) -> CausalForestDML:
...
def _causal_tree_rules(model: CausalForestDML, x: np.ndarray, feature_names: list[str], max_depth: int) -> str:
    interpreter = SingleTreeCateInterpreter(max_depth=max_depth, min_samples_leaf=40, include_model_uncertainty=False)
    interpreter.interpret(model, x)
    tree_model = interpreter.tree_model_
...
def run(config_path: Path) -> None:
    ...
    year = cfg["year"]
    sample_size = cfg.get("sample_size", 100000)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Roda uma árvore causal por tratamento (RENDA e SCORE_CULT_PAIS)")
    parser.add_argument("--config", default="config/causal_trees.yml", type=Path)
